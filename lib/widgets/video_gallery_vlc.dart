...
class VideoGalleryVlcPlayer extends StatefulWidget {
  const VideoGalleryVlcPlayer({Key? key}) : super(key: key);

  @override
  State<VideoGalleryVlcPlayer> createState() => _VideoGalleryVlcPlayerState();
}

class _VideoGalleryVlcPlayerState extends State<VideoGalleryVlcPlayer> {
  ...
  void _onVlcPlayerChanged() async {
    ...
    setState(() {
     ...
     // _duration = dur?.inMilliseconds.toDouble() > 0 ? dur!.inMilliseconds.toDouble() : 1;
  ...
  Widget _buildVideoPlayer({bool fullscreen = false}) {
    if (_vlcController == null) return const SizedBox();
